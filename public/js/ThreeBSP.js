...
(function () {
  ...
  window.ThreeBSP = (function () {
    ...
    ThreeBSP.prototype.toTree = function (geometry) {
      const polygons = [];
      ...
      if (!(geometry instanceof THREE.BufferGeometry)) {
        throw new Error('Geometry is not an instance of BufferGeometry');
      }

      // 从BufferGeometry中提取顶点、法线和UV数据
      ...
      for (let i = 0; i < indexArray.length; i += 3) {
        const vertices = [];
        for (let j = 0; j < 3; j++) {
          const index = indexArray[i + j] * 3;
          const position = new THREE.Vector3(
            positions[index],
            positions[index + 1],
            positions[index + 2]
          );
          position.applyMatrix4(this.matrix); // 应用矩阵
          ...
          const uv = uvs.length
            ? new THREE.Vector2(uvs[(index / 3) * 2], uvs[(index / 3) * 2 + 1])
            : new THREE.Vector2();

          vertices.push(new ThreeBSP.Vertex(position, normal, uv));
        }
